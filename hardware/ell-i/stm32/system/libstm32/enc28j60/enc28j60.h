...
typedef struct enc_rx_packet_header {
    uint16_t   rx_next;
    uint16_t   rx_length;      /* Length of the received frame */
    uint16_t   rx_status;
} enc_rx_packet_header_t;

extern int  enc_reg_get(enc_reg_t reg);
extern void enc_reg_set(enc_reg_t reg, int value);
extern void enc_reg_bitop(enc_spi_op_t op, enc_reg_t reg, int mask);

extern int  enc_packet_receive(enc_buf_value_t *buffer, enc_buf_len_t len);
extern void enc_packet_send(enc_buf_value_t *buffer, enc_buf_len_t len);

extern void enc_init(const uint8_t mac_address[ETH_ADDRESS_LEN]);
